<x-app-layout>
    <x-slot name="title">
        {{ __('Add penalty template') }} - {{ $championship->title }}
    </x-slot>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-zinc-800 leading-tight flex gap-2">
            ...
            <span>/</span>
            <span>{{ __('Add penalty template') }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('championships.penalties.store', $championship) }}">
                @csrf
