...
class DetermineParticipantZoneTest extends TestCase
{
    ...
    public function test_region_updated_at_participant_update(): void
    {
        config(['races.registration.form' => 'complete']);
        Notification::fake();
        ...
        app(UpdateParticipantRegistration::class)($race, $participant, [
            'bib' => $participant->bib,
            'category' => $category->ulid,
            ...$this->generateValidDriver(['driver_residence_province' => '']),
            'driver_residence_province' => 'TO',
            ...$this->generateValidVehicle(),
        ]);

        $this->assertSame(ItalianRegion::PIEMONTE, $participant->fresh()->region);
    }

    public function test_out_of_zone_updated_at_participant_update_when_zone_configured(): void
    {
        config(['races.registration.form' => 'complete']);
        Notification::fake();
