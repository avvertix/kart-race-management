<x-app-layout>
    <x-slot name="title">
        {{ $participant->full_name }} - {{ __('Assigned tire set') }} - {{ $race->title }}
    </x-slot>
    <x-slot name="header">
        <h2 class="font-semibold text-4xl text-zinc-800 leading-tight flex gap-2">
            ...
            <span>{{ __('Assigned tire set') }}</span>
        </h2>
        <div class="prose prose-zinc mb-6">
            <p class="">{{ __('You must bring this to the official at the entrance of the race track for technical verification.') }}</p>
        </div>
