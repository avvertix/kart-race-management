<x-app-layout>
    <x-slot name="title">
        {{ __('Copy categories') }} - {{ $championship->title }}
    </x-slot>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-zinc-800 leading-tight flex gap-2">
            ...
            <span>/</span>
            <span>{{ __('Copy categories') }}</span>
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('championships.categories.store-copy', $championship) }}">
                @csrf

                <div class="md:grid md:grid-cols-3 md:gap-6">
                    <div class="md:col-span-1">
                        <div class="px-4 sm:px-0">
                            <h3 class="text-lg font-medium leading-6 text-zinc-900">{{ __('Copy categories from another championship') }}</h3>
                            <p class="mt-1 text-sm text-zinc-600">
                                {{ __('Select a championship to copy all categories from. Categories will be copied with their settings and tire references. Missing tires will be automatically created.') }}
                            </p>
                        </div>
                    </div>
                                        ...
                                        <div class="flex">
                                            <div class="ml-3">
                                                ...
                                                <div class="mt-2 text-sm text-yellow-700">
                                                    <p>{{ __('There are no other championships with categories to copy from.') }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
